...
- `preflight.js` — stage-agnostic environment and repository checks, run before any interview; `--stage pr` also enforces the `ship:approved` gate and reports an existing open PR for the branch (CLI rejects a flag-shaped token as another flag's value, e.g. `--cwd --quiet`, mirroring config.js)
